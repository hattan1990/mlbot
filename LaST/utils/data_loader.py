...
class Dataset_BTC(Dataset):
    def __init__(self, root_path, flag='train', size=None, features='MS', data_path='ETTm1.csv',
                 use_decoder_tokens=False, date_period1='2022-01-01 00:00', date_period2='2023-01-01 00:00',date_period3='2023-04-01 00:00',
                 target='cl', inverse =False , cols = None, scale=True, timeenc=0, freq='t', option=0, add_feature=""):
...
class Dataset_ETT_minute(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.inverse:
            if self.features == 'MS':
                self.data_y = np.expand_dims(df_data.values[border1:border2, -1], 1)
            else:
                self.data_y = df_data.values[border1:border2]
        else:
            if self.features == 'MS':
                self.data_y = np.expand_dims(data[border1:border2, -1], 1)
            else:
                self.data_y = data[border1:border2]
        self.data_stamp = data_stamp
...
class Dataset_Elec(Dataset):
    ...
    def __read_data__(self):
        ...
        data_scaler.fit(data_stamp)
        data_scaler.transform(data_stamp)
